...
function OnTvShow() {
    ...
    const [onTvShow, setOnTvShows] = useState([]);

    useEffect(() => {
        fetch(TMDB_URL)
            .then((res) => res.json())
            .then((data) => {
                setOnTvShows(data.results || []);
            })
            .catch((error) => {
        ...
        <>
            <section className='mb-9'>
                <div className='mx-6 md:mx-8 lg:mx-11 pb-2 lg:pb-3 flex items-center justify-between'>
                    <h2 className='text-[16px] md:text-lg xl:text-xl font-bold text-white'>On TV Shows</h2>
                </div>
                <div>
