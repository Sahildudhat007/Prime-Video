...
const TMDB_URL = `https://api.themoviedb.org/3/movie/upcoming?api_key=${API_KEY}&language=en-US&page=1`;

function UpcomingMovies() {

    const [uocomingMovies, setUpcomingMovies] = useState([]);
        ...
        <>
            <section className='mb-9'>
                <div className='mx-6 md:mx-8 lg:mx-11 pb-2 lg:pb-3 flex items-center justify-between'>
                    <h2 className='text-[16px] md:text-lg xl:text-xl font-bold text-white'>Upcoming Movies</h2>
                </div>
                <div>
                    <ul className='pl-6 md:pl-8 lg:pl-11'>
                        ...
                        >
                            {uocomingMovies.map((movie) => (
                                <SwiperSlide key={movie.id} className="group relative text-white w-[280px] cursor-pointer">
                                        ...
                                        <div className="bg-[#111] rounded-md overflow-hidden transition-transform duration-300 ease-in-out shadow-lg">
                                            <div className="relative">
                                                ...
                                                <img
                                                    ...
                                                    alt="Prime"
                                                    className="absolute bottom-2 right-2 w-12"
                                                />
                                            </div>
                                            <div className="px-3 py-2">
                                                <h3 className="text-sm font-semibold truncate">{movie ? movie.title : ""}</h3>
                                            </div>
                                            ...
                                            <div className="absolute inset-0 bg-black/75 p-4 flex flex-col justify-end opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out rounded-md">
                                                ...
                                                <div className="flex gap-2 flex-wrap justify-center items-center text-sm mt-2">
                                                    ...
                                                    <span className="bg-[#333] px-2 py-0.5 rounded-full">{movie ? movie.release_date : ""}</span>
